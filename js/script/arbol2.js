...
animationbuildTree = (count) => {
  ...
  $("#cabeceraTabla").append(`<tr>
        <th class="animate__animated animate__fadeIn" scope="col">Character</th>
        <th class="animate__animated animate__fadeIn" scope="col">Frequency</th>
    </tr>`);
...
colorear = (simbolo) => {
  ...
  $("#cuerpoTabla2").append(`<tr>
    <td class="animate__animated animate__fadeIn">${String.fromCharCode(
      simbolo
    )}</td>
...
function cicloColorear(i) {
  ...
  setTimeout(function () {
    ...
    if (i == hojas.length - 1) $("#CodiM").show(); //si llegó a todas las hojas, entonces aparece el boton de la siguiente seccion
  }, 3000 * i);
  return true;
}
/**
...
function colorearTodas() {
  ...
  $("#cabeceraTabla2").append(`<tr>
        ...
        <th class="animate__animated animate__fadeIn" scope="col">Code</th>
    </tr>`);
  //vacia el cuerpo de la tabla
  $("#cuerpoTabla2").empty();
  ...
  hojas.forEach((c) => {
    ...
    let miPrimeraPromise = new Promise((resolve, reject) => {
      if (cicloColorear(i) == true)
        //si es verdadera, se cumplio la promesa
        resolve(true);
      else reject(error); //cancelar en caso de error
    });
...
$("#CodiM").click(() => {
  ...
  for (var i = 0; i < msg.length; i++) {
    typewriter
      ...
      .pauseFor(1000)
      .deleteChars(1)
      .typeString("<strong>" + codigos[msg[i]] + "</strong>")
      .start();
  }
  ...
  const myChart = new Chart(ctx, {
    ...
    data: {
      labels: ["message original", "Codification"], //sus etiquetas
      datasets: [
        {
          label: "# de Bytes",
  ...
  document.getElementById("percentage").innerHTML =
    "Percentage compression: " + porcentaje.toFixed(3) + "%";
...
$("#DecoM").click(() => {
  ...
  for (var i = 0; i < msg.length; i++)
    $("#typeDeco").append("<span id=c" + i + ">" + codigos[msg[i]] + "</span>");
